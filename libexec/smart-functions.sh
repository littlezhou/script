...
declare -a SMART_SUBCMD_USAGE
declare -a SMART_OPTION_USAGE
...
{
  if [[ -n "${SMART_SHELL_SCRIPT_DEBUG}" ]]; then
    echo "DEBUG: $*" 1>&2
  fi
}
...
function hadoop_add_subcommand
{
  ...
  SMART_SUBCMD_USAGE[${SMART_SUBCMD_USAGE_COUNTER}]="${subcmd}@${text}"
  ((SMART_SUBCMD_USAGE_COUNTER=SMART_SUBCMD_USAGE_COUNTER+1))
}
...
function hadoop_add_option
{
  ...
  SMART_OPTION_USAGE[${SMART_OPTION_USAGE_COUNTER}]="${option}@${text}"
  ((SMART_OPTION_USAGE_COUNTER=SMART_OPTION_USAGE_COUNTER+1))
}
...
function hadoop_reset_usage
{
  SMART_SUBCMD_USAGE=()
  SMART_OPTION_USAGE=()
  SMART_SUBCMD_USAGE_COUNTER=0
  SMART_OPTION_USAGE_COUNTER=0
}
...
function hadoop_generate_usage
{
  ...
  if [[ -n "${SMART_OPTION_USAGE_COUNTER}"
        && "${SMART_OPTION_USAGE_COUNTER}" -gt 0 ]]; then
    haveoptions=true
    optstring=" [OPTIONS]"
  fi

  if [[ -n "${SMART_SUBCMD_USAGE_COUNTER}"
        && "${SMART_SUBCMD_USAGE_COUNTER}" -gt 0 ]]; then
    havesubs=true
  ...
  if [[ "${haveoptions}" = true ]]; then
    ...
    hadoop_generic_columnprinter "${SMART_OPTION_USAGE[@]}"
  fi
  ...
  if [[ "${havesubs}" = true ]]; then
    ...
    hadoop_generic_columnprinter "${SMART_SUBCMD_USAGE[@]}"
    echo ""
...
function hadoop_bootstrap
{
  ...
  if [[ -n "${DEFAULT_LIBEXEC_DIR}" ]]; then
    hadoop_error "WARNING: DEFAULT_LIBEXEC_DIR ignored. It has been replaced by SMART_DEFAULT_LIBEXEC_DIR."
  fi

  # By now, SMART_LIBEXEC_DIR should have been defined upstream
  # We can piggyback off of that to figure out where the default
  # SMART_FREFIX should be.  This allows us to run without
  # SMART_HOME ever being defined by a human! As a consequence
  # SMART_LIBEXEC_DIR now becomes perhaps the single most powerful
  # env var within Hadoop.
  if [[ -z "${SMART_LIBEXEC_DIR}" ]]; then
    hadoop_error "SMART_LIBEXEC_DIR is not defined.  Exiting."
    exit 1
  fi
  SMART_DEFAULT_PREFIX=$(cd -P -- "${SMART_LIBEXEC_DIR}/.." >/dev/null && pwd -P)
  SMART_HOME=${SMART_HOME:-$SMART_DEFAULT_PREFIX}
  export SMART_HOME
  ...
  #
  SMART_COMMON_DIR=${SMART_COMMON_DIR:-"lib"}
  SMART_COMMON_LIB_JARS_DIR=${SMART_COMMON_LIB_JARS_DIR:-"share/hadoop/common/lib"}
  SMART_COMMON_LIB_NATIVE_DIR=${SMART_COMMON_LIB_NATIVE_DIR:-"lib/native"}
  ...
  SMART_TOOLS_HOME=${SMART_TOOLS_HOME:-${SMART_HOME}}
  SMART_TOOLS_DIR=${SMART_TOOLS_DIR:-"share/hadoop/tools"}
  SMART_TOOLS_LIB_JARS_DIR=${SMART_TOOLS_LIB_JARS_DIR:-"${SMART_TOOLS_DIR}/lib"}

  # by default, whatever we are about to run doesn't support
  # daemonization
  SMART_SUBCMD_SUPPORTDAEMONIZATION=false

  # by default, we have not been self-re-execed
  SMART_REEXECED_CMD=false

  SMART_SUBCMD_SECURESERVICE=false

  # This is the default we claim in smart-env.sh
  JSVC_HOME=${JSVC_HOME:-"/usr/bin"}

  # usage output set to zero
  hadoop_reset_usage

  export SMART_OS_TYPE=${SMART_OS_TYPE:-$(uname -s)}

  # defaults
  export SMART_OPTS=${SMART_OPTS:-"-Djava.net.preferIPv4Stack=true"}
  hadoop_debug "Initial SMART_OPTS=${SMART_OPTS}"
}
...
function hadoop_find_confdir
{
  ...
  # installs.
  if [[ -e "${SMART_HOME}/conf/smart-env.sh" ]]; then
    conf_dir="conf"
  else
    conf_dir="etc/hadoop"
  fi
  export SMART_CONF_DIR="${SMART_CONF_DIR:-${SMART_HOME}/${conf_dir}}"

  hadoop_debug "SMART_CONF_DIR=${SMART_CONF_DIR}"
}

## @description  Validate ${SMART_CONF_DIR}
## @audience     public
## @stability    stable
## @replaceable  yes
...
function hadoop_verify_confdir
{
  ...
  if [[ ! -f "${SMART_CONF_DIR}/log4j.properties" ]]; then
    hadoop_error "WARNING: log4j.properties is not found. SMART_CONF_DIR may be incomplete."
  fi
}

## @description  Import the smart-env.sh settings
## @audience     private
## @stability    evolving
## @replaceable  no
function hadoop_exec_hadoopenv
{
  if [[ -z "${SMART_ENV_PROCESSED}" ]]; then
    if [[ -f "${SMART_CONF_DIR}/smart-env.sh" ]]; then
      export SMART_ENV_PROCESSED=true
      # shellcheck source=./hadoop-common-project/hadoop-common/src/main/conf/smart-env.sh
...
{
  if [[ -e "${SMART_CONF_DIR}/hadoop-user-functions.sh" ]]; then
    # shellcheck disable=SC1090
    . "${SMART_CONF_DIR}/hadoop-user-functions.sh"
  fi
}

## @description  Read the user's settings.  This provides for users to
## @description  override and/or append smart-env.sh. It is not meant
...
function hadoop_import_shellprofiles
{
  ...
  if [[ -d "${SMART_LIBEXEC_DIR}/shellprofile.d" ]]; then
    files1=(${SMART_LIBEXEC_DIR}/shellprofile.d/*.sh)
    hadoop_debug "shellprofiles: ${files1[*]}"
  else
    hadoop_error "WARNING: ${SMART_LIBEXEC_DIR}/shellprofile.d doesn't exist. Functionality may not work."
  fi

  if [[ -d "${SMART_CONF_DIR}/shellprofile.d" ]]; then
    files2=(${SMART_CONF_DIR}/shellprofile.d/*.sh)
  fi

  # enable bundled shellprofiles that come
  # from hadoop-tools.  This converts the user-facing SMART_OPTIONAL_TOOLS
  # to the SMART_TOOLS_OPTIONS that the shell profiles expect.
  # See dist-tools-hooks-maker for how the example SMART_OPTIONAL_TOOLS
  # gets populated into smart-env.sh

  for i in ${SMART_OPTIONAL_TOOLS//,/ }; do
    hadoop_add_entry SMART_TOOLS_OPTIONS "${i}"
  done
...
{
  # Some of these are also set in smart-env.sh.
  # we still set them here just in case smart-env.sh is
  # broken in some way, set up defaults, etc.
  #
  # but it is important to note that if you update these
  # you also need to update smart-env.sh as well!!!

  CLASSPATH=""
  hadoop_debug "Initialize CLASSPATH"
  echo "SMART_COMMON_HOME=" ${SMART_COMMON_HOME}
  echo "${SMART_HOME}"
  echo "${SMART_HOME}/${SMART_COMMON_DIR}"

  if [[ -z "${SMART_COMMON_HOME}" ]] &&
  [[ -d "${SMART_HOME}/${SMART_COMMON_DIR}" ]]; then
    export SMART_COMMON_HOME="${SMART_HOME}"
  fi

  # default policy file for service-level authorization
  SMART_POLICYFILE=${SMART_POLICYFILE:-"hadoop-policy.xml"}

  # define SMART_HDFS_HOME
  if [[ -z "${SMART_HDFS_HOME}" ]] &&
     [[ -d "${SMART_HOME}/${HDFS_DIR}" ]]; then
    export SMART_HDFS_HOME="${SMART_HOME}"
  fi

  # define SMART_YARN_HOME
  if [[ -z "${SMART_YARN_HOME}" ]] &&
     [[ -d "${SMART_HOME}/${YARN_DIR}" ]]; then
    export SMART_YARN_HOME="${SMART_HOME}"
  fi

  # define SMART_MAPRED_HOME
  if [[ -z "${SMART_MAPRED_HOME}" ]] &&
     [[ -d "${SMART_HOME}/${MAPRED_DIR}" ]]; then
    export SMART_MAPRED_HOME="${SMART_HOME}"
  fi

  if [[ ! -d "${SMART_COMMON_HOME}" ]]; then
    hadoop_error "ERROR: Invalid SMART_COMMON_HOME"
    exit 1
  fi

  echo "SMART_COMMON_HOME=" ${SMART_COMMON_HOME}
  echo "SMART_COMMON_DIR=" ${SMART_COMMON_DIR}

  # if [[ ! -d "${SMART_HDFS_HOME}" ]]; then
  #   hadoop_error "ERROR: Invalid SMART_HDFS_HOME"
  #   exit 1
  # fi

  # if [[ ! -d "${SMART_YARN_HOME}" ]]; then
  #   hadoop_error "ERROR: Invalid SMART_YARN_HOME"
  #   exit 1
  # fi

  # if [[ ! -d "${SMART_MAPRED_HOME}" ]]; then
  #   hadoop_error "ERROR: Invalid SMART_MAPRED_HOME"
  #   exit 1
  # fi
  ...
  USER=${USER:-$(id -nu)}
  SMART_IDENT_STRING=${SMART_IDENT_STRING:-$USER}
  SMART_LOG_DIR=${SMART_LOG_DIR:-"${SMART_HOME}/logs"}
  SMART_LOGFILE=${SMART_LOGFILE:-hadoop.log}
  SMART_LOGLEVEL=${SMART_LOGLEVEL:-INFO}
  SMART_NICENESS=${SMART_NICENESS:-0}
  SMART_STOP_TIMEOUT=${SMART_STOP_TIMEOUT:-5}
  SMART_PID_DIR=${SMART_PID_DIR:-/tmp}
  SMART_ROOT_LOGGER=${SMART_ROOT_LOGGER:-${SMART_LOGLEVEL},console}
  SMART_DAEMON_ROOT_LOGGER=${SMART_DAEMON_ROOT_LOGGER:-${SMART_LOGLEVEL},RFA}
  SMART_SECURITY_LOGGER=${SMART_SECURITY_LOGGER:-INFO,NullAppender}
  SMART_SSH_OPTS=${SMART_SSH_OPTS-"-o BatchMode=yes -o StrictHostKeyChecking=no -o ConnectTimeout=10s"}
  SMART_SECURE_LOG_DIR=${SMART_SECURE_LOG_DIR:-${SMART_LOG_DIR}}
  SMART_SECURE_PID_DIR=${SMART_SECURE_PID_DIR:-${SMART_PID_DIR}}
  SMART_SSH_PARALLEL=${SMART_SSH_PARALLEL:-10}
}
...
function hadoop_populate_workers_file
{
  ...
  if [[ -f "${workersfile}" ]]; then
    SMART_WORKERS="${workersfile}"
  elif [[ -f "${SMART_CONF_DIR}/${workersfile}" ]]; then
    SMART_WORKERS="${SMART_CONF_DIR}/${workersfile}"
  else
...
## @description  Connect to ${SMART_WORKERS} or ${SMART_WORKER_NAMES}
...
function hadoop_connect_to_hosts
{
  ...
  if [[ -n "${SMART_WORKERS}" && -n "${SMART_WORKER_NAMES}" ]] ; then
    hadoop_error "ERROR: Both SMART_WORKERS and SMART_WORKER_NAMES were defined. Aborting."
    exit 1
  ...
  if [[ -e '/usr/bin/pdsh' ]]; then
    if [[ -z "${SMART_WORKER_NAMES}" ]] ; then
      # if we were given a file, just let pdsh deal with it.
      # shellcheck disable=SC2086
      PDSH_SSH_ARGS_APPEND="${SMART_SSH_OPTS}" pdsh \
      -f "${SMART_SSH_PARALLEL}" -w ^"${worker_file}" $"${@// /\\ }" 2>&1
    else
      # no spaces allowed in the pdsh arg host list
      # shellcheck disable=SC2086
      tmpslvnames=$(echo ${SMART_WORKER_NAMES} | tr -s ' ' ,)
      PDSH_SSH_ARGS_APPEND="${SMART_SSH_OPTS}" pdsh \
        -f "${SMART_SSH_PARALLEL}" \
        -w "${tmpslvnames}" $"${@// /\\ }" 2>&1
    fi
  else
    if [[ -z "${SMART_WORKER_NAMES}" ]]; then
      SMART_WORKER_NAMES=$(sed 's/#.*$//;/^$/d' "${worker_file}")
    fi
    hadoop_connect_to_hosts_without_pdsh "${params}"
  fi
}

## @description  Connect to ${SMART_WORKER_NAMES} and execute command
...
function hadoop_connect_to_hosts_without_pdsh
{
  # shellcheck disable=SC2124
  local params="$@"
  local workers=(${SMART_WORKER_NAMES})
  for (( i = 0; i < ${#workers[@]}; i++ ))
  do
    if (( i != 0 && i % SMART_SSH_PARALLEL == 0 )); then
      wait
    fi
    # shellcheck disable=SC2086
...
{
  # shellcheck disable=SC2086
  hadoop_add_param SMART_SHELL_PROFILES $1 $1
}
...
function hadoop_add_common_to_classpath
{
  ...
  if [[ -z "${SMART_COMMON_HOME}"
    || -z "${SMART_COMMON_DIR}"
    || -z "${SMART_COMMON_LIB_JARS_DIR}" ]]; then
    hadoop_debug "COMMON_HOME=${SMART_COMMON_HOME}"
    hadoop_debug "COMMON_DIR=${SMART_COMMON_DIR}"
    hadoop_debug "COMMON_LIB_JARS_DIR=${SMART_COMMON_LIB_JARS_DIR}"
    hadoop_error "ERROR: SMART_COMMON_HOME or related vars are not configured."
    exit 1
  fi

  # developers
  if [[ -n "${SMART_ENABLE_BUILD_PATHS}" ]]; then
    hadoop_add_classpath "${SMART_COMMON_HOME}/hadoop-common/target/classes"
  fi

  hadoop_add_classpath "${SMART_COMMON_HOME}/${SMART_COMMON_LIB_JARS_DIR}"'/*'
  hadoop_add_classpath "${SMART_COMMON_HOME}/${SMART_COMMON_DIR}"'/*'
}
...
function hadoop_add_to_classpath_tools
{
  ...
  if [[ -f "${SMART_LIBEXEC_DIR}/tools/${module}.sh" ]]; then
    # shellcheck disable=SC1090
    . "${SMART_LIBEXEC_DIR}/tools/${module}.sh"
  else
    hadoop_error "ERROR: Tools helper ${SMART_LIBEXEC_DIR}/tools/${module}.sh was not found."
  fi
...
{
  # Add the user-specified SMART_CLASSPATH to the
  # official CLASSPATH env var if SMART_USE_CLIENT_CLASSLOADER
  # is not set.
  # Add it first or last depending on if user has
  # set env-var SMART_USER_CLASSPATH_FIRST
  ...
  if [[ -n "${SMART_CLASSPATH}" ]]; then
    # I wonder if Java runs on VMS.
    for idx in $(echo "${SMART_CLASSPATH}" | tr : '\n'); do
      array[${c}]=${idx}
      ((c=c+1))
    done
    ...
    if [[ -z "${SMART_USE_CLIENT_CLASSLOADER}" ]]; then
      if [[ -z "${SMART_USER_CLASSPATH_FIRST}" ]]; then
        for ((i=0; i<=j; i++)); do
...
function hadoop_os_tricks
{
  ...
  SMART_IS_CYGWIN=false
  case ${SMART_OS_TYPE} in
    Darwin)
      if [[ -z "${JAVA_HOME}" ]]; then
    ...
    Linux)
      ...
      # NOTE! SMART_ALLOW_IPV6 is a developer hook.  We leave it
      # undocumented in smart-env.sh because we don't want users to
...
function hadoop_finalize_libpaths
{
  if [[ -n "${JAVA_LIBRARY_PATH}" ]]; then
    ...
    hadoop_add_param SMART_OPTS java.library.path \
...
{
  if [[ -n "${SMART_HEAPSIZE_MAX}" ]]; then
    if [[ "${SMART_HEAPSIZE_MAX}" =~ ^[0-9]+$ ]]; then
      SMART_HEAPSIZE_MAX="${SMART_HEAPSIZE_MAX}m"
    fi
    hadoop_add_param SMART_OPTS Xmx "-Xmx${SMART_HEAPSIZE_MAX}"
  fi

  # backwards compatibility
  if [[ -n "${SMART_HEAPSIZE}" ]]; then
    if [[ "${SMART_HEAPSIZE}" =~ ^[0-9]+$ ]]; then
      SMART_HEAPSIZE="${SMART_HEAPSIZE}m"
    fi
    hadoop_add_param SMART_OPTS Xmx "-Xmx${SMART_HEAPSIZE}"
  fi

  if [[ -n "${SMART_HEAPSIZE_MIN}" ]]; then
    if [[ "${SMART_HEAPSIZE_MIN}" =~ ^[0-9]+$ ]]; then
      SMART_HEAPSIZE_MIN="${SMART_HEAPSIZE_MIN}m"
    fi
    hadoop_add_param SMART_OPTS Xms "-Xms${SMART_HEAPSIZE_MIN}"
  fi
}
...
## @description  Adds the SMART_CLIENT_OPTS variable to
## @description  SMART_OPTS if SMART_SUBCMD_SUPPORTDAEMONIZATION is false
## @audience     public
## @stability    stable
## @replaceable  yes
function hadoop_add_client_opts
{
  if [[ "${SMART_SUBCMD_SUPPORTDAEMONIZATION}" = false
     || -z "${SMART_SUBCMD_SUPPORTDAEMONIZATION}" ]]; then
    hadoop_debug "Appending SMART_CLIENT_OPTS onto SMART_OPTS"
    SMART_OPTS="${SMART_OPTS} ${SMART_CLIENT_OPTS}"
  fi
}
...
{
  hadoop_translate_cygwin_path SMART_LOG_DIR
  hadoop_add_param SMART_OPTS hadoop.log.dir "-Dhadoop.log.dir=${SMART_LOG_DIR}"
  hadoop_add_param SMART_OPTS hadoop.log.file "-Dhadoop.log.file=${SMART_LOGFILE}"
  hadoop_translate_cygwin_path SMART_HOME
  export SMART_HOME
  hadoop_add_param SMART_OPTS hadoop.home.dir "-Dhadoop.home.dir=${SMART_HOME}"
  hadoop_add_param SMART_OPTS hadoop.id.str "-Dhadoop.id.str=${SMART_IDENT_STRING}"
  hadoop_add_param SMART_OPTS hadoop.root.logger "-Dhadoop.root.logger=${SMART_ROOT_LOGGER}"
  hadoop_add_param SMART_OPTS hadoop.policy.file "-Dhadoop.policy.file=${SMART_POLICYFILE}"
  hadoop_add_param SMART_OPTS hadoop.security.logger "-Dhadoop.security.logger=${SMART_SECURITY_LOGGER}"
}
...
{
  hadoop_add_classpath "${SMART_CONF_DIR}" before
...
function hadoop_finalize
{
  ...
  hadoop_translate_cygwin_path SMART_HOME
  hadoop_translate_cygwin_path SMART_CONF_DIR
  hadoop_translate_cygwin_path SMART_COMMON_HOME
  hadoop_translate_cygwin_path SMART_HDFS_HOME
  hadoop_translate_cygwin_path SMART_YARN_HOME
  hadoop_translate_cygwin_path SMART_MAPRED_HOME
}
...
function hadoop_verify_secure_prereq
{
  ...
  if ! hadoop_privilege_check && [[ -z "${SMART_SECURE_COMMAND}" ]]; then
...
function hadoop_setup_secure_service
{
  ...
  SMART_PID_DIR=${SMART_SECURE_PID_DIR}
  SMART_LOG_DIR=${SMART_SECURE_LOG_DIR}
}
...
function hadoop_verify_piddir
{
  ...
  hadoop_mkdir "${SMART_PID_DIR}"
  touch "${SMART_PID_DIR}/$$" >/dev/null 2>&1
  if [[ $? -gt 0 ]]; then
    hadoop_error "ERROR: Unable to write in ${SMART_PID_DIR}. Aborting."
    exit 1
  fi
  rm "${SMART_PID_DIR}/$$" >/dev/null 2>&1
}
...
function hadoop_verify_logdir
{
  ...
  hadoop_mkdir "${SMART_LOG_DIR}"
  touch "${SMART_LOG_DIR}/$$" >/dev/null 2>&1
  if [[ $? -gt 0 ]]; then
    hadoop_error "ERROR: Unable to write in ${SMART_LOG_DIR}. Aborting."
    exit 1
  fi
  rm "${SMART_LOG_DIR}/$$" >/dev/null 2>&1
}
...
function hadoop_java_exec
{
  ...
  hadoop_debug "Final SMART_OPTS: ${SMART_OPTS}"
...
function hadoop_start_secure_daemon_wrapper
{
  ...
  if [[ -f "${daemonpidfile}" ]]; then
    ...
    renice "${SMART_NICENESS}" $(cat "${daemonpidfile}" 2>/dev/null) >/dev/null 2>&1
    if [[ $? -gt 0 ]]; then
  ...
  # capture the ulimit output
  su "${SMART_SECURE_USER}" -c 'bash -c "ulimit -a"' >> "${jsvcoutfile}" 2>&1
  #shellcheck disable=SC2086
  if ! ps -p $! >/dev/null 2>&1; then
    return 1
  fi
...
function hadoop_stop_daemon
{
  ...
  if [[ -f "${pidfile}" ]]; then
    ...
    sleep "${SMART_STOP_TIMEOUT}"
    if kill -0 "${pid}" > /dev/null 2>&1; then
      hadoop_error "WARNING: ${cmd} did not stop gracefully after ${SMART_STOP_TIMEOUT} seconds: Trying to kill with kill -9"
...
## @description and if SMART_SECURE_CLASSNAME is defined
## @audience     public
## @stability    stable
## @replaceable  yes
## @param        command
...
function hadoop_detect_priv_subcmd
{
  ...
  if [[ -z "${SMART_SECURE_CLASSNAME}" ]]; then
...
function hadoop_need_reexec
{
  ...
  if [[ "${SMART_REEXECED_CMD}" = true ]]; then
    return 1
  fi
...
## @description  Add custom (program)_(command)_OPTS to SMART_OPTS.
...
function hadoop_subcommand_opts
{
  ...
  # SMART_NAMENODE_OPTS -> HDFS_NAMENODE_OPTS

  depvar="SMART_${ucommand}_OPTS"
  ...
  if [[ -n ${!uvar} ]]; then
    hadoop_debug "Appending ${uvar} onto SMART_OPTS"
    SMART_OPTS="${SMART_OPTS} ${!uvar}"
    return 0
  fi
}

## @description  Add custom (program)_(command)_SECURE_EXTRA_OPTS to SMART_OPTS.
...
## @description  SMART_PARSE_COUNTER to set number the
...
{
  SMART_DAEMON_MODE="default"
  SMART_PARSE_COUNTER=0
  ...
  while true; do
    ...
    case $1 in
      --buildpaths)
        SMART_ENABLE_BUILD_PATHS=true
        shift
        ((SMART_PARSE_COUNTER=SMART_PARSE_COUNTER+1))
      ;;
      --config)
        shift
        confdir=$1
        shift
        ((SMART_PARSE_COUNTER=SMART_PARSE_COUNTER+2))
        if [[ -d "${confdir}" ]]; then
          SMART_CONF_DIR="${confdir}"
        elif [[ -z "${confdir}" ]]; then
      ...
      --daemon)
        ...
        shift
        ((SMART_PARSE_COUNTER=SMART_PARSE_COUNTER+2))
        if [[ -z "${SMART_DAEMON_MODE}" || \
          ! "${SMART_DAEMON_MODE}" =~ ^st(art|op|atus)$ ]]; then
      ...
      --reexec)
        ...
        SMART_REEXECED_CMD=true
        ((SMART_PARSE_COUNTER=SMART_PARSE_COUNTER+1))
      ;;
      --workers)
        shift
        # shellcheck disable=SC2034
        SMART_WORKER_MODE=true
        ((SMART_PARSE_COUNTER=SMART_PARSE_COUNTER+1))
      ;;
      *)
        break
  ...
  hadoop_debug "hadoop_parse: asking caller to skip ${SMART_PARSE_COUNTER}"
}
...
function hadoop_generic_java_subcmd_handler
{
  ...
  if hadoop_detect_priv_subcmd "${SMART_SHELL_EXECNAME}" "${SMART_SUBCMD}"; then
    ...
    secureuser=$(hadoop_build_custom_subcmd_var "${SMART_SHELL_EXECNAME}" "${SMART_SUBCMD}" SECURE_USER)
...
echo "1111111111111111111"
  ...
  if [[ "${SMART_SUBCMD_SECURESERVICE}" = true ]]; then
    ...
    hadoop_subcommand_secure_opts "${SMART_SHELL_EXECNAME}" "${SMART_SUBCMD}"

    hadoop_verify_secure_prereq
    hadoop_setup_secure_service
    priv_outfile="${SMART_LOG_DIR}/privileged-${SMART_IDENT_STRING}-${SMART_SUBCMD}-${HOSTNAME}.out"
    priv_errfile="${SMART_LOG_DIR}/privileged-${SMART_IDENT_STRING}-${SMART_SUBCMD}-${HOSTNAME}.err"
    priv_pidfile="${SMART_PID_DIR}/privileged-${SMART_IDENT_STRING}-${SMART_SUBCMD}.pid"
    daemon_outfile="${SMART_LOG_DIR}/hadoop-${SMART_SECURE_USER}-${SMART_IDENT_STRING}-${SMART_SUBCMD}-${HOSTNAME}.out"
    daemon_pidfile="${SMART_PID_DIR}/hadoop-${SMART_SECURE_USER}-${SMART_IDENT_STRING}-${SMART_SUBCMD}.pid"
  else
...
echo "222222222222222222222"
  ...
  if [[ "${SMART_DAEMON_MODE}" != "default" ]]; then
    SMART_ROOT_LOGGER="${SMART_DAEMON_ROOT_LOGGER}"
    if [[ "${SMART_SUBCMD_SECURESERVICE}" = true ]]; then
      SMART_LOGFILE="hadoop-${SMART_SECURE_USER}-${SMART_IDENT_STRING}-${SMART_SUBCMD}-${HOSTNAME}.log"
    else
